...
- **P1.114** [x] **FIXED v0.2.6** — Null-safe model_permissions filters: `cp_country_code.is_not_null()` guard on geo branch, `.fill_null(False)` on book-exclusion `str.contains` result. Null book_code now correctly passes the exclusion filter; null cp_country_code conservatively fails the geo filter. `engine/classifier.py:1010-1030`.
...
- **P1.158** [x] **FIXED v0.2.6** — `lookup_collateral_haircut` now returns the longest maturity-band haircut (`5y_plus` under CRR / `10y_plus` under B31) when `residual_maturity_years is None`, instead of falsy-or'ing to 5.0 → mid-band. Both govt_bond and corp_bond branches updated in `data/tables/haircuts.py`. Art. 224(3) conservative fallback now applied uniformly.
...
- **P1.169** [x] **FIXED v0.2.6** — B31 ECRA short-term institution Table 4 corrected to CQS 1-3 = 20%, CQS 4-5 = 50%, CQS 6 = 150% per PRA PS1/26 Art. 120(2). `data/tables/b31_risk_weights.py` table values + comment updated; `engine/sa/namespace.py:_b31_append_institution_maturity_branches` now uses three-way `cqs<=3 / cqs<=5 / else` chain mirroring CRR pattern. Existing CQS 4/5 short-term test pins refreshed to the correct 50%.
