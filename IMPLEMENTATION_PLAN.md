...
- **P1.94** [~] **PARTIAL — sub-item (a) FIXED v0.2.27** — B31 currency mismatch 1.5x: 150% cap fixed v0.1.192; auto-detection wired; sub-item (a) `is_hedged` flag now gates the multiplier per Art. 123B(2) hedge exemption. New `is_hedged` Bool on `LOAN_SCHEMA` (`data/schemas.py`); `apply_currency_mismatch_multiplier` in `engine/sa/namespace.py:1888` AND-gates `mismatch_applies` with `~is_hedged`. Pinned by `tests/acceptance/basel31/test_p1_94a_is_hedged_gates_currency_mismatch.py` (7 tests across hedged + unhedged + cross-arm RW-delta). **Sub-items remaining:** (b) 90%-coverage hedge test (Art. 123B(2)); (d) revolving instalment rule (123B(2A)); (e) pre-2027 portfolio fallback (123B(3)); (f) scope too broad — catches non-RRE CRE but 123B is retail (h) / residential RE (i) only; (g) CR5 pre-multiplier RW reporting + OF 02.00 row 0380 memo. **Effort: S** | Ref: PRA PS1/26 Art. 123B
- **P1.95** [x] **FIXED v0.2.23** — `build_institution_guarantor_rw_expr` extended with `scra_grade_col` kwarg dispatching unrated B31 institution guarantors to `B31_SCRA_RISK_WEIGHTS` (A=40, A_ENHANCED=30, B=75, C=150) / `B31_SCRA_SHORT_TERM_RISK_WEIGHTS`; null SCRA → Grade C 150% (CRE20.21 conservative). CRM guarantee join projects `guarantor_scra_grade`; SA `_build_guarantor_rw_expr` passes the kwarg + defensive null-fill. Pinned by `tests/acceptance/basel31/test_p1_95_b31_unrated_inst_guarantor_scra.py` (22 tests). | Ref: PRA PS1/26 Art. 121/235
...
- **P1.122** [~] **PARTIAL — sub-claim (c) FIXED v0.2.22** — Short-term institution guarantor now uses Art. 120(2) Table 4 (CQS 1-3=20%, CQS 4-5=50%, CQS 6=150%) when borrower exposure original_maturity_years ≤ 0.25 (3m). New `INSTITUTION_SHORT_TERM_RISK_WEIGHTS_B31_ECRA` dict + `build_institution_guarantor_rw_expr` short_term_flag_col param + `_inst_guarantor_short_term` derivation in `apply_guarantee_substitution`. Pinned by `tests/acceptance/basel31/test_p1_122_short_term_institution_guarantor_substitution.py` (6 tests). **Sub-claims (a) corporate CQS 3 = 75% and (b) unrated institution SCRA grades remain open.** | Ref: PRA PS1/26 Art. 122/121/235
- **P1.123** [x] **FIXED (silent fix) — closed v0.2.22 (already implemented in commit `e5bbdbd` batch 20260509-1726)**. `(1+HE)` gross-up wired in `engine/crm/collateral.py`; pinned by `tests/acceptance/crr/test_p1_123_art_223_5_fccm_exposure_volatility_haircut.py` (13 tests). Plan entry was stale. | Ref: CRR Art. 223(5)
- **P1.127** [x] **FIXED v0.2.24** — Art. 159 Pool B regression-guard. Hierarchy resolver passes `ava_amount` / `other_own_funds_reductions` through to per-exposure outputs; classifier `is_defaulted` is now `cp_default_status | row-is_defaulted | beel>0`. Pinned by `tests/acceptance/crr/test_p1_127_art_159_pool_b_ava_regression_guard.py` (3 methods). | Ref: CRR Art. 159(1)
...
- **P1.154** [x] **FIXED v0.2.22 (regression-guard added; engine already correct)**. `ExposureClass.INTERNATIONAL_ORGANISATION` (`domain/enums.py:96`) + SA branches at `engine/sa/namespace.py:912/1094` route Art. 118 IO 0% and B3.1 Art. 117(1)(a) Table 2B CQS2=30% non-named MDB correctly. New B3.1 acceptance test `tests/acceptance/basel31/test_p1_154_b31_art_118_international_organisation_class.py` (9 tests) pins both branches. CRR-side acceptance test pre-existed. | Ref: CRR/PRA PS1/26 Art. 112(1)(e), Art. 117-118
...
- **P1.179** [x] **FIXED v0.2.25 (regression-guard, no engine change)** — Bullet was stale: `get_firb_lgd_table(is_basel_3_1=...)` already exists at `data/tables/firb_lgd.py:450` and dispatches to CRR / B3.1 tables; `get_firb_lgd_table_for_framework` does the same for the dict surface. Naming-and-dispatch contract pinned by `tests/unit/data_tables/test_p1_179_firb_lgd_naming_regression.py` (6 tests) — covers CRR DataFrame defaults, B3.1 `is_fse` split, schema delta, dict helper values, cross-helper consistency, and re-export object identity through `data/tables/__init__.py`. | Ref: CRR Art. 161, PRA PS1/26 Art. 161
...
- **P1.163** [x] **FIXED v0.2.28** — Corrected stale Basel 3.1 PD floor values in the docstring of `_pd_floor_expression` at `engine/irb/formulas.py:64-65`: retail mortgage `0.05% → 0.10%` (Art. 163(1)(b)) and QRRE transactors `0.03% → 0.05%, revolvers: 0.10%` (Art. 163(1)(c)). Existing `Corporate/SME: 0.05%`, `Retail other: 0.05%`, and `CRE30.55` / `Art. 163(1)` references preserved. `CalculationConfig.basel_3_1().pd_floors` constants were already correct (`retail_mortgage = Decimal("0.0010")`, `retail_qrre_transactor = Decimal("0.0005")`); docstring-only with no calculation impact. Pinned by `tests/unit/irb/test_p1_163_pd_floor_docstring.py` (9 tests). | Ref: PRA PS1/26 Art. 163(1)(a-c); BCBS CRE30.55
- **P1.166** [x] **FIXED v0.2.27** — Stale docstring on `EquityType.CIU` at `domain/enums.py:481` corrected from "150% CRR SA / 250% listed or 400% unlisted B31 SA" to PRA PS1/26 Art. 132(2) wording (1,250% fallback under both CRR and B31 SA when neither look-through Art. 132A(1) nor mandate-based Art. 132A(2) applies). Adds post-class assignment `EquityType.CIU.__doc__ = ...` so the corrected text is exposed at runtime — Python enum machinery does not propagate trailing-string docstrings on members. Plan-bullet's `equity/calculator.py:21` pointer was stale; that line was already correct from P1.119/v0.1.184. Pinned by `tests/unit/test_p1_166_ciu_fallback_docstring.py` (4 tests). | Ref: PRA PS1/26 Art. 132(2)
- **P1.168** [x] **FIXED v0.2.28** — Corrected stale BCBS-aligned `100%` references in two docstrings in `data/tables/b31_risk_weights.py`: module docstring (line 16) `"CQS5: 100%" → "CQS5: 150%"` and `get_b31_combined_cqs_risk_weights` docstring (line 390) `"CQS5=100%" → "CQS5=150%"`. PRA PS1/26 Art. 122(1) Table 6 retains corporate CQS 5 = 150% as a deviation from BCBS CRE20.42 (which reduced it to 100%). Constant `B31_CORPORATE_RISK_WEIGHTS[5] = Decimal("1.50")` at line 151 unchanged; inline comment at line 143 already correct. (Plan-bullet's `:327` line ref was stale — actual second offending docstring is at line 390.) Pinned by `tests/unit/data_tables/test_p1_168_corporate_cqs5_docstring.py` (5 tests). | Ref: PRA PS1/26 Art. 122(1) Table 6 (PRA Deviation)
...
- **P1.185** [x] **FIXED v0.2.28** — Replaced fabricated CET1/leverage thresholds in `SCRAGrade.B` docstring at `domain/enums.py` with the qualitative criterion of Art. 121(1)(b): *"Substantial credit risk but meets published minimum requirements (excluding buffers) (Art. 121(1)(b)) -> 75% RW (>3m), 50% (<=3m)"*. Numeric thresholds in the previous docstring were never in the regulation. Installed via post-class `SCRAGrade.B.__doc__ = "..."` (mirrors EquityType.CIU precedent from P1.166); Python enum machinery does not capture trailing-string literals on members. Lookup logic and `B31_SCRA_RISK_WEIGHTS["B"] = Decimal("0.75")` unchanged. Out-of-scope: stale comment with same fabricated text at `b31_risk_weights.py:182` (separate item). Pinned by `tests/unit/test_p1_185_scra_grade_b_docstring.py` (8 tests). | Ref: PRA PS1/26 Art. 121(1)(b)
...
- **P2.12** [x] **FIXED v0.2.25** — Column 0020 "Exposures deducted from own funds" added to both `CRR_C07_COLUMNS` and `B31_C07_COLUMNS` between 0010 and 0030 in `reporting/corep/templates.py`; `_compute_c07_values` in `reporting/corep/generator.py` emits col 0020 from optional input field `own_funds_deduction_amount` via `_col_sum_eager`, defaulting to `None` when absent (mirrors col 0035 on-bs-netting precedent). Schema-side addition of `own_funds_deduction_amount` to `FACILITY_SCHEMA`/`LOAN_SCHEMA` and the regulatory `0040 = 0010 − 0020 − 0030 − 0035` formula change deferred. Pinned by `tests/unit/test_corep.py::TestC0700Col0020` (4 tests). | Ref: PRA PS1/26 Annex II §C 07.00 / §OF 07.00 col 0020; CRR Art. 36/56/66
...
- **P2.17** [x] **FIXED v0.2.29** — CRR retail payroll/pension 35% RW now applied per CRR Art. 123 second subparagraph (CRR2 / Reg (EU) 2019/876, F68). `_apply_crr_risk_weight_overrides` in `engine/sa/namespace.py` now branches on `is_payroll_loan` ahead of the flat-75% catch-all (mirrors B3.1 ordering). 35% scalar reused from `B31_RETAIL_PAYROLL_LOAN_RW = Decimal("0.35")` (`data/tables/b31_risk_weights.py:252`) — identical under PRA PS1/26 Art. 123(3)(a-b) and CRR Art. 123 second subparagraph; cross-framework reuse documented inline. Pinned by `tests/acceptance/crr/test_p2_17_crr_payroll_loan_35pct_rw.py` (2 payroll @ 35% + 1 control @ 75%, anti-regression guards). | Ref: CRR Art. 123 (CRR2 amendment)
...
- **P2.18** [x] **FIXED v0.2.26 (regression-guard, no engine change)** — `revaluation_frequency_days` field already on `COLLATERAL_SCHEMA` (data/schemas.py:331); `engine/crm/haircuts.py:144-207` already applies Art. 226(1) `sqrt((NR + T_m - 1) / T_m)` symmetrically to both collateral and FX haircuts. Plan-bullet was stale. New B3.1 acceptance scenario `tests/acceptance/basel31/test_p2_18_art_226_1_b31_secured_lending_fx.py` (7 tests) pins the corner that was previously uncovered: 20-day secured-lending T_m, USD govt-bond collateral vs GBP loan, weekly reval (N_R=5). Asserts `ead_final ≈ 239,427.40` and a regression guard `ead_final ≠ 227,279.22` (the no-Art.226(1) counterfactual). Builder + parquets at `tests/fixtures/p2_18/`. | Ref: CRR / PRA PS1/26 Art. 226(1)
...
- **P2.22** [x] **FIXED v0.2.24 (regression-guard, no engine change)** — Under current CRR scalars (SME tier-1=0.7619, tier-2=0.85, infra=0.75), `pl.min_horizontal(SME, infra)` is observationally identical to Art. 501(2)'s "infra replaces SME" substitution rule because the invariant `0.75 < 0.7619 ≤ SME_blended ≤ 0.85` holds. No capital understatement exists today. Behaviour pinned by `tests/acceptance/crr/test_scenario_crr_f_supporting_factors.py::TestP222SMEInfraOverlapSubstitution` with anti-assertion `supporting_factor != 0.7619` so a future calibration change that violates the invariant fails the test loudly. | Ref: CRR Art. 501(2) 2nd subpara, Art. 501a(1)
...
- **P2.24** [x] **FIXED v0.2.26** — `_dedup_org_mappings` helper in `engine/hierarchy.py` performs first-row-wins dedup on `child_counterparty_reference` via `unique(..., keep="first", maintain_order=True)` and emits one `CalculationError(code="DQ004", severity=WARNING, category=DATA_QUALITY, counterparty_reference=<child>, field_name="child_counterparty_reference")` per duplicated child. Dedup'd frame feeds `_build_ultimate_parent_lazy`, `_enrich_counterparties_with_hierarchy`, and `CounterpartyLookup.parent_mappings` — fixes the silent row fan-out at the (former) `hierarchy.py:491-501` join (now lines 580-591). Pinned by `tests/unit/test_hierarchy.py::TestOrgMappingDuplicateChild` (2 tests: duplicate-trigger + control arm). Contracts allowlist bumped 393,394 → 405,406 for the unrelated `_build_rating_inheritance_lazy` `.over()` calls. | Ref: contracts/errors.py:165 ERROR_DUPLICATE_KEY="DQ004"
...
- **P2.34** [x] **FIXED v0.2.21** — `validate_aggregated_bundle` added in `contracts/validation.py` with bounds OUT001-OUT004 (RW≤12.5, RW≥0, RWA≥0, EAD non-null); pinned by `tests/contracts/test_aggregated_bundle_validation.py`.
- **P2.35** [x] **FIXED v0.2.25** — `_resolve_graph_eager` in `engine/hierarchy.py` now flags truncated chains via a new `truncated:Boolean` column (detection: `depth == max_depth AND current in parent_of` post-loop); `_build_counterparty_lookup` materialises one `CalculationError(code=ERROR_HIERARCHY_DEPTH="HIE003", severity=WARNING, category=HIERARCHY)` per truncated entity, then drops the helper column to preserve the published `CounterpartyLookup.ultimate_parent_mappings` schema. Resolver still surfaces deepest reachable parent — non-fatal. Pinned by `tests/unit/test_hierarchy_max_depth.py` (4 tests on a 12-node chain CP_DEPTH_C0..CP_DEPTH_C11). | Ref: internal hierarchy contract; CLAUDE.md § Error Handling
...
- **P2.42** [x] **FIXED v0.2.23** — `_of_02_01_row` at `src/rwa_calc/reporting/corep/generator.py:2988-2993` now sets col 0030 = `modelled_rwa + sa_rwa` per Annex II §1.3.2. Stale companion test `test_u_trea_equals_modelled` removed; new positive test `test_u_trea_is_sum_of_modelled_and_sa` added. Pinned by `tests/unit/test_corep.py::TestOF0201CreditRiskRow` (9 tests). | Ref: COREP OF 02.01 / PRA PS1/26 Annex II §1.3.2
- **P2.43** [x] **FIXED v0.2.29** — PSM LGD source switch exposed via `IRBPermissions.psm_lgd_source: Literal["option_i", "option_ii"] = "option_ii"` (default preserves historical behaviour). `_apply_parameter_substitution` in `engine/irb/guarantee.py` now branches LGD_covered: option (i) → `pl.col("lgd")` (post-`apply_firb_lgd` borrower-seniority-correct supervisory LGD); option (ii) → existing per-row guarantor F-IRB selection. `_apply_no_better_than_direct_floor` refactored to take `psm_lgd_expr` + `direct_lgd_expr` separately so the Art. 160(4) NBD comparison always uses option (ii) regardless of switch (regulatory invariant). `_adjust_expected_loss` mirrors the same branch (Art. 236(1A)(b)). `CalculationConfig.irb_permissions` widened from `field(init=False)` to `IRBPermissions | None = None` with `__post_init__` deriving the framework-default (enables `dataclasses.replace`; runtime always non-None). Pinned by `tests/acceptance/basel31/test_p2_43_psm_lgd_source_switch.py` (option_ii RW≈0.619 / RWA≈619k; option_i RW≈1.161 / RWA≈1.16M; cross-arm RWA delta ≈541k). | Ref: PRA PS1/26 Art. 236(1)(a)(i)
...
- **P6.18** [x] **FIXED v0.2.21** — `_load_file_optional` now catches `FileNotFoundError` (silent) separately from any other `Exception` (DQ007 `WARNING`/`DATA_QUALITY` `CalculationError` + lazy `logger.warning`); `_load_file` (required-file path) untouched; pinned by `tests/unit/test_loader_optional_error_handling.py`.
- **P6.19** [x] **FIXED v0.2.23** — `DATA_SOURCES` now includes `DataSourceFile(id="ciu_holdings", relative_path=Path("equity/ciu_holdings"), requirement=OPTIONAL, ...)` and `DataSourceConfig.from_registry()` populates `ciu_holdings_file` for both parquet and csv extensions. Pinned by `tests/unit/config/test_p6_19_data_sources_ciu_holdings.py` (7 tests). | Ref: CRR/PRA PS1/26 Art. 132 CIU look-through
- **P6.20** [x] **FIXED v0.2.21** — `_create_config()` now passes `base_currency=self.base_currency` into both `CalculationConfig.crr()` and `.basel_3_1()` factories; both factories accept and forward the kwarg. Default `"GBP"` preserved. Pinned by `tests/unit/api/test_service_base_currency.py`.
...
- **P6.26** [x] **FIXED v0.2.29** — TODO marker closed with explanatory comment + module-level constant `_FACILITY_QRRE_COUPLED_COLUMNS = ("is_revolving", "is_qrre_transactor", "facility_limit", "facility_termination_date")` extracted in `engine/hierarchy.py`. Site analysis (Site A `_undrawn_select_expressions` = select on facility frame; Site B `_propagate_facility_qrre_columns` = join+coalesce on unified exposures) showed the two sites are NOT duplicating the same expression — different stages, different operations, different null semantics. A merge would either need an awkward two-mode helper or force Site A to do an unnecessary self-join. The constant makes the four-name coupling explicit without forcing false code-sharing. Allowlisted in `scripts/arch_check.py` (engine-internal coupling marker, mirrors `NULLABLE_PARTITION_KEYS` precedent) and `tests/contracts/test_no_raw_over_on_nullable_keys.py` (line allowlist bumped 405,406 → 420,421 for the unrelated `_build_rating_inheritance_lazy` `.over()` calls). Pinned by `tests/unit/test_p6_26_qrre_coupling_constant.py` (constant existence, TODO removal, four-column propagation regression — Sites A + B both covered). Pure non-functional refactor: no `pl.col` / `pl.coalesce` expressions touched. | Ref: `src/rwa_calc/engine/hierarchy.py`
