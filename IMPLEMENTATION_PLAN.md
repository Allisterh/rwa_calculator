...
**Last updated:** 2026-05-03 (P2.47 + P2.48 added — Art. 136/137/138 ECAI-grade / MEIP / second-best lookup from D4.74; CR8 flow rows + signed-convention emission from D4.82; both surfaced by docs batch b89fe59)
**Package version:** 0.1.202 (pyproject.toml) | **Test suite:** 5,329 selectable (5,329 total, 11 deselected benchmarks)
...
- **P1.99** [x] **FIXED v0.1.202 (2026-04-18)** — CRR Art. 120(2) Table 4 short-term rated institution now applied. Added `INSTITUTION_SHORT_TERM_RISK_WEIGHTS_CRR` (CQS 1-3 = 20%, CQS 4-5 = 50%, CQS 6 = 150%) in `data/tables/crr_risk_weights.py` and a new `.when()` branch in `engine/sa/calculator.py` keyed on `residual_maturity_years <= 0.25`. CRR Art. 120(2) imposes no domestic currency condition (distinct from Art. 119(2)). Bundled with P1.121. **Effort: S** | Ref: CRR Art. 120(1)-(2)
...
- **P1.121** [x] **FIXED v0.1.202 (2026-04-18)** — CRR Art. 121(3) unrated institution short-term 20% now applied. Added `INSTITUTION_SHORT_TERM_UNRATED_RW_CRR = 0.20` and a new `.when()` branch keyed on `original_maturity_years <= 0.25` (Art. 121(3) uses ORIGINAL effective maturity, not residual — consistent with the P1.133 short-term PSE/institution fix). Art. 121(6) sovereign floor still applies to FX exposures via the existing `_apply_sovereign_floor_for_institutions` pass. Art. 121(3) imposes no domestic currency restriction. Bundled with P1.99. **Effort: S** | Ref: CRR Art. 121(3)
...
- **P1.149** [x] **FIXED (2026-04-18)** — CRR Institution CQS 2 risk weight corrected. Renamed `INSTITUTION_RISK_WEIGHTS_UK` → `INSTITUTION_RISK_WEIGHTS_B31_ECRA` (values 30%/40% were always PRA PS1/26 Basel 3.1 ECRA, not a CRR UK deviation); added `INSTITUTION_RISK_WEIGHTS_CRR` with correct CRR Art. 120 Table 3 values (CQS 2 = 50%, unrated = 100%). Replaced `use_uk_deviation` (keyed on base currency `== "GBP"`) with `config.is_basel_3_1` (keyed on framework) across `engine/sa/calculator.py`, `engine/irb/guarantee.py`, and `engine/equity/calculator.py`. Caught additional same-root-cause bug at `irb/guarantee.py:269` where unrated institution guarantors were hard-coded 40% regardless of framework — now 100% under CRR, 40% under B31. Acceptance scenarios CRR-A4 (0.30→0.50) and CRR-D4 (0.58→0.70) updated. **Effort: S** | Ref: CRR Art. 120 Table 3
- **P1.150** [x] **FIXED v0.1.198 (2026-04-18)** — HVCRE Good slotting EL rate corrected. `data/tables/b31_slotting.py` and `data/tables/crr_slotting.py` HVCRE GOOD entries changed from `Decimal("0.008")` (0.8%) to `Decimal("0.004")` (0.4%) per PRA PS1/26 Art. 158(6) Table B (Appendix 1 p.108) — HVCRE row is flat 0.4% across A/B/C/D, collapsing the subgrade differentiation that non-HVCRE retains. Halves the EL shortfall for HVCRE Good exposures. UK CRR Art. 158 was omitted by SI 2021/1078, so PRA PS1/26 is the only extant UK source — fix applied symmetrically to both framework tables. Test rename + new regression guard `test_hvcre_good_diverges_from_non_hvcre_long_maturity` in `tests/unit/test_slotting_el_rates.py`. Spec admonition + changelog updated. **Effort: S** | Ref: PRA PS1/26 Art. 158(6) Table B
...
- **P1.155** [x] **FIXED v0.1.200 (2026-04-18)** — PRA PS1/26 Art. 224 Table 1 B31 haircut corrections. `BASEL31_COLLATERAL_HAIRCUTS` in `data/tables/haircuts.py` had 9 stale values (P1.155 originally cited 4; PDF verification found 5 more in the same table). Corrections verified against ps126app1.pdf p.203: sovereign CQS 2-3 3_5y 4%→3% and 10y+ 12%→6% (5-band split is not a penal re-scale for well-rated sovereigns); corp/institution CQS 1 1_3y/3_5y/5_10y 4/6/10%→3/4/6%; corp/institution CQS 2-3 1_3y/3_5y/5_10y/10y+ 6/8/15/15%→4/6/12/20%. The 10y+ CQS 2-3 correction (15%→20%) is a capital *increase*; the other 8 corrections were over-haircuts. Test class `TestBasel31BondHaircuts` parametrized into 13 cases. **Effort: S** | Ref: PRA PS1/26 Art. 224 Table 1
...
- **P1.133** [x] **FIXED v0.1.201** — Short-term PSE/institution treatment now keys on `original_maturity_years`. `_SA_INPUT_CONTRACT` extended with `original_maturity_years` / `value_date` / `maturity_date`; `calculate_branch` derives `original_maturity_years` from `(maturity_date - value_date)/365.0` when the column is null so hierarchy-supplied facility data flows through without requiring a new schema column. Five call-sites updated: B31 PSE Art. 116(3), B31 ECRA rated institution Art. 120(2)/(2A) (incl. 6m trade-goods carve-out), B31 SCRA unrated institution Art. 121(3), CRR PSE Art. 116(3), and Art. 121(6) trade-goods sovereign floor exception. 8 regression tests in `test_pse_risk_weights.py` (CRR + B31 seasoned-vs-fresh) and `test_b31_sa_risk_weights.py` (ECRA/SCRA seasoned-vs-fresh). CRR ECRA rated institution Art. 120(2) legitimately keeps `residual` semantics in the regulation but has no dedicated code branch — tracked as P1.99/P1.121. **Effort: S** | Ref: CRR Art. 116(3), PRA PS1/26 Art. 120(2)/(2A), Art. 121(3)/(6)
