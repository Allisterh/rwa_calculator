...
The full citation -> function matrix is published at [citation-matrix.md](citation-matrix.md). It's generated by `scripts/generate_citation_matrix.py` from the live `@cites(...)` decorators. Regenerate after annotation changes:

```bash
uv run python scripts/generate_citation_matrix.py
```

The script invokes `uv run watchfire matrix --format markdown` once for the CRR instrument and once for the PS instrument, stitches the rendered tables together, and writes `docs/development/citation-matrix.md`. Wire it into CI on push-to-master if you want the published site to track every commit.
