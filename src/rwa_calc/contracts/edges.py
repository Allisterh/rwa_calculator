...
"""SACalculator.calculate_branch output (CRR Art. 112-134)."""
...
"""IRBCalculator.calculate_branch output (CRR Art. 151-166)."""
...
"""SlottingCalculator.calculate_branch output (CRR Art. 153(5))."""
...
AGGREGATOR_EXIT_EDGE: EdgeContract = EdgeContract(
    ...
    columns={
        ...
        "el_pre_adjustment": EdgeColumn(dtype=pl.Float64),
        "el_shortfall": EdgeColumn(dtype=pl.Float64),
        # Optional-with-injection: supplied by the equity concat on every
        # normal run (null on non-equity rows); injected as typed null when
        # the equity stage degraded, so an equity failure no longer nukes
        # the non-equity results via a self-inflicted seal violation.
...
"""OutputAggregator combined results — the reporting input contract."""
