...
# reconcilable components (see data/schemas.RECONCILABLE_COMPONENTS) and declares
# the composite/custom join key. These are configuration, not regulatory data, so
# they live here alongside the other frozen ``*Config`` dataclasses rather than in
# data/tables. Tolerance defaults come from the component registry; per-component
# overrides live on ``ComponentMapping`` (single source).
